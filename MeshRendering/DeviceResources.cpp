...
void DX::DeviceResources::CreateWindowSizeDependentResources()
{
	...
	// Create the Texture2D for use in the depth stencil view for 3D rendering
	CD3D11_TEXTURE2D_DESC1 depthStencilDesc;
	depthStencilDesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;			// 24 bits store the depth value - 8 bits store the stencil value
	...
	depthStencilDesc.BindFlags = D3D11_BIND_DEPTH_STENCIL;				// Enable binding to the output merger stage as the depth stencil buffer
	depthStencilDesc.SampleDesc.Count = 1;								// Describes how to sample when MSAA is enabled - it is not enabled in this case, so the count should be 1
	depthStencilDesc.SampleDesc.Quality = 0;							// MSAA is not enabled, so sampling quality should be 0
	depthStencilDesc.MiscFlags = 0;										// No miscellaneous flags
	depthStencilDesc.CPUAccessFlags = 0;								// CPU can not read or write to the buffer (must be 0 because Usage = DEFAULT_USAGE)
	depthStencilDesc.Usage = D3D11_USAGE_DEFAULT;						// Enable GPU read and write only
	depthStencilDesc.TextureLayout = D3D11_TEXTURE_LAYOUT_UNDEFINED;	// Texture Layout is undefined and will be determined by the driver

	winrt::com_ptr<ID3D11Texture2D1> depthStencil;
	DX::ThrowIfFailed(
		m_d3dDevice->CreateTexture2D1(
			&depthStencilDesc,	// pointer to the description structure
			nullptr,			// pointer to initial data
			depthStencil.put()	// pointer to a pointer to the memory block that will be created
		)
	);

	// Create the Depth Stencil View to bind to the output merger stage for 3D rendering
	CD3D11_DEPTH_STENCIL_VIEW_DESC depthStencilViewDesc(
		D3D11_DSV_DIMENSION_TEXTURE2D,	// viewDimension	- The resource will be accessed as a 2D texture
		DXGI_FORMAT_UNKNOWN,			// format			- Resource data format, format is unknown
		0U,								// mipSlice			- First mipmap level to use
		0U,								// firstArraySlice	- Not relevant because we are NOT using a Texture2DArray
		0U,								// arraySize		- Not relevant because we are NOT using a Texture2DArray
		0U								// flags			- 0 specifies the resource is not read only (i.e. both depth and stencil values may be written during a rendering pass)
	);

	DX::ThrowIfFailed(
		m_d3dDevice->CreateDepthStencilView(
			depthStencil.get(),			// pointer to the resource that will serve as the depth-stencil surface
			&depthStencilViewDesc,		// pointer to depth stencil view description struct
			m_d3dDepthStencilView.put()	// pointer to a pointer to a depth stencil view to be created
		)
	);
	...
	m_screenViewport = CD3D11_VIEWPORT(
		0.0f,							// top left X
		0.0f,							// top left Y
		m_d3dRenderTargetSize.Width,	// width
		m_d3dRenderTargetSize.Height,	// height
		0.0f,							// min depth - for scaling Z coordination, usually not necessary
		1.0f							// max depth
	);

	m_d3dContext->RSSetViewports(
		1,					// Only 1 viewport to set
		&m_screenViewport	// pointer to the viewport
	);

	// Set the rasterizer state
	D3D11_RASTERIZER_DESC1 rasterizerStateDesc;			
	rasterizerStateDesc.FillMode = D3D11_FILL_SOLID;	// Completely fill the interior of the fragments generated by the vertices
	rasterizerStateDesc.CullMode = D3D11_CULL_FRONT;	// Cull all front facing triangles
	rasterizerStateDesc.FrontCounterClockwise = true;	// Counter clockwise triangles are considered to be front facing
	rasterizerStateDesc.DepthBias = false;				// Not performing shadow mapping or any other algorithm that requires a depth bias
	rasterizerStateDesc.DepthBiasClamp = 0;				// Not performing shadow mapping or any other algorithm that requires a depth bias
	rasterizerStateDesc.SlopeScaledDepthBias = 0;		// Not performing shadow mapping or any other algorithm that requires a depth bias
	rasterizerStateDesc.DepthClipEnable = true;			// Clip the geometry to the near and far clipping planes
	rasterizerStateDesc.ScissorEnable = false;			// Disable the scissor test
	rasterizerStateDesc.MultisampleEnable = false;		// Disable MSAA
	rasterizerStateDesc.AntialiasedLineEnable = false;	// Disable antialiasing with line primitives (there are no line primitives being rendered)
	rasterizerStateDesc.ForcedSampleCount = 0;			// The sample count that is forced while UAV rendering or rasterizing (If >1, see documentation: https://docs.microsoft.com/en-us/windows/win32/api/d3d11_1/ns-d3d11_1-d3d11_rasterizer_desc1)
